...
router.get("/file-submissions/:id", async (req, res): Promise<void> => {
  ...
  if (user.role === "student" && row.studentId !== user.id) { res.status(403).json({ error: "Forbidden" }); return; }
...
router.patch("/file-submissions/:id", async (req, res): Promise<void> => {
  ...
  if (user.role === "student" && existing.studentId !== user.id) { res.status(403).json({ error: "Forbidden" }); return; }
  if (user.role === "student" && existing.status !== "pending" && existing.status !== "revision_requested") {
    res.status(400).json({ error: "Cannot edit a reviewed submission" }); return;
  }

  const { title, description, fileUrl, fileName, fileType, fileSize } = req.body;
  const [row] = await db.update(fileSubmissionsTable)
    .set({ title, description, fileUrl, fileName, fileType, fileSize, ...(user.role === "student" ? { status: "pending" } : {}) })
